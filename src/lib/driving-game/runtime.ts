...
import { ENGINE_WORKLET_SOURCE } from "./audio/engine-worklet-source";
import { TIRE_WORKLET_SOURCE } from "./audio/tire-worklet-source";
import { DRIVING_PROFILES, type DrivingProfile } from "./driving-profiles";
import { GAME_MAPS, type GameMapDefinition } from "./maps";
...
export function startDrivingGame(root: HTMLElement, options: DrivingGameOptions = {}) {
  ...
  const DRIVING = DRIVING_PROFILES[options.drivingProfile ?? "balanced"];
  ...
  const overheadCamera = new THREE.OrthographicCamera(-20, 20, 20, -20, 0.1, map.environment.cameraFar);
...
function buildWorld(scene: THREE.Scene, obstacles: Obstacle[], map: GameMapDefinition) {
  const grassMaterial = new THREE.MeshStandardMaterial({ color: map.environment.grass, roughness: 1 });
  const ground = new THREE.Mesh(new THREE.PlaneGeometry(map.groundSize, map.groundSize), grassMaterial);
  ground.rotation.x = -Math.PI / 2;
  ground.receiveShadow = true;
  scene.add(ground);

  const roadMaterial = new THREE.MeshStandardMaterial({ color: map.environment.road, roughness: 0.92 });
  ...
  roadSegments.forEach((road) => {
    ...
    if (road.width >= road.depth) {
      for (let x = road.x - road.width / 2 + 5; x <= road.x + road.width / 2 - 5; x += 7) {
        addRoadMark(scene, x, road.z, 3.3, 0.12, lineMaterial);
      }
    } else {
      for (let z = road.z - road.depth / 2 + 5; z <= road.z + road.depth / 2 - 5; z += 7) {
        addRoadMark(scene, road.x, z, 0.12, 3.3, lineMaterial);
      }
    }
  });
  ...
  map.parkingLots.forEach((parkingLot) => {
    const lot = new THREE.Mesh(
      new THREE.BoxGeometry(parkingLot.width, 0.1, parkingLot.depth),
      roadMaterial,
    );
    lot.position.set(parkingLot.x, 0.04, parkingLot.z);
...
function createCarAudio(DRIVING: DrivingProfile): CarAudio | null {
