...
yaml_array = YAML.safe_load(File.read(yaml_dir), permitted_classes: [Time, Date, DateTime])
file.close

yaml_array.each_with_index do |note, index|
  next unless note.key?('prettyTime') == false
  ...
  note['initialTimestamp'] = Time.parse(note['initialTimestamp']) if note['initialTimestamp'].class == String
  puts note
  ENV['TZ'] = 'US/Eastern'
