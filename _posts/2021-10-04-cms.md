...
- Layer 1: Locally (and in real-time) build AST documents with a block-editor collection and layout GUI and support for Layer 2.3
...
But, layout building should also be *optional*—it starts with editing collections and collection items. *Document* editing. And if documents are made of blocks, *block* editing.
...
*Sidenote*: Atom (Syndication Format and Publishing Protocol) should be colloquially named Follow, Subscribe, Collection, or Feed protocol. [Atom > RSS](https://danielmiessler.com/blog/atom-rss-why-we-should-just-call-them-feeds-instead-of-rss-feeds/).

But, the default document editing experience (HTML) is garbage compared to the interfaces provided by social networks—most everyone will never pick HTML as a language to build documents.
...
Aside: If there are publishing servers, could you try to provide free content hosting by inserting advertisements into the RSS feed? And then “pay for no ads” on both sides of the market (either publishers or subscribers can pay). Or maybe [sponsorships](https://daringfireball.net/feeds/sponsors/)?
